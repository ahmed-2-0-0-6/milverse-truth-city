...
const NEW_CASES: FeedScenario[] = [
...

  {
    ...
    sender: { name: "Friend Sara", relationship: "Instagram DM", voice: "hopeful, saving for a wedding, sends screenshots" },
    senderMotive: "She saw a celebrity she loves 'personally recommending' this. It felt like insider access.",
    opener: "Girl look at this — [famous actor] herself is telling people about it. I'm thinking of putting in $200, want to go halves?",
    forward: {
    ...
    actions: [
      { id: "check-celeb-handle", label: "Open the celebrity's REAL verified account", tool: "check_source", result: "The real verified account has never posted about this platform. Her team has posted warnings about fake endorsements using her likeness.", decisive: true },
      { id: "reverse-image", label: "Reverse-search the celebrity photo used", tool: "reverse_image", result: "The image is a paparazzi photo from 2022, unrelated to any investment product. Reused without permission.", decisive: true },
      { id: "cross-check", label: "Search the platform name in news + regulators", tool: "cross_check", result: "SECP (Pakistan) and multiple foreign regulators list this platform on their public investor-alert pages. Documented Ponzi pattern.", decisive: true },
      { id: "check-guarantee", label: "Sanity-check the 'guaranteed returns'", tool: "check_date", result: "No legitimate regulated investment product guarantees monthly returns. That phrase alone is a regulator red flag worldwide." },
    ],
    truthNote: "Celebrity investment endorsements on social ads are almost always scams — the celebrity's face is stolen, not licensed. 'Guaranteed monthly returns' is not a real financial product; it is the signature of a Ponzi.",
    respectfulScript: "Sara ruk — main ne check kiya. Actress ke asli verified account pe iss product ka zikr tak nahi hai, aur unki team khud ne warning post ki hai ke unki tasveer misuse ho rahi hai. SECP ne bhi iss platform ko investor alert list pe rakha hua hai. Paisa mat lagana yaar — asli investing pe baat karte hain properly.",
    inspiredBy: {
      patternName: "Fake Celebrity Investment Endorsement",
      country: "Global",
      year: "2018–ongoing",
      whatHappened: "Scam trading platforms buy social ads featuring stolen images of celebrities, athletes, or business figures with fabricated quotes. Victims deposit and are shown a fake dashboard climbing until they try to withdraw. Regulators worldwide publish standing investor alerts for these operators.",
      prevention: [
...

  {
    ...
    sender: { name: "Uncle Javed", relationship: "Family group", voice: "loves 'breaking news', trusts what looks like Dawn" },
    senderMotive: "He thinks he's the first in the group to catch a huge story. The URL genuinely goes to dawn.com.",
    opener: "Just in — Dawn ne khud publish kiya hai. Yeh sun ke shock lag gaya. Share karo sab ko.",
    forward: {
    ...
    actions: [
      { id: "check-date", label: "Scroll to the article's published date", tool: "check_date", result: "The article is real — but published in May 2019. Nearly seven years old. The sender's 'today' caption is added on top.", decisive: true },
      { id: "cross-check", label: "Cross-check today's economy news", tool: "cross_check", result: "No current outlet is reporting a devaluation today. Dawn's homepage right now doesn't feature this piece.", decisive: true },
      { id: "check-source", label: "Open dawn.com fresh and search the headline", tool: "check_source", result: "The story sits in Dawn's 2019 archive exactly as shown. It is Dawn's real journalism — just from a very different economic moment.", decisive: true },
      { id: "reverse-image", label: "Reverse-image the shared preview card", tool: "reverse_image", result: "The preview image is Dawn's real thumbnail from 2019. It is not doctored — the deception is the missing date." },
    ],
    truthNote: "The outlet is real. The reporting is real. The DATE is the whole lie. Recirculating a real old article as breaking news is one of the fastest-spreading formats — nobody thinks to fact-check a real outlet.",
    respectfulScript: "Uncle Dawn ki khabar bilkul asli hai — lekin yeh article May 2019 ka hai, aaj ka nahi. Aaj ki economy ki khabrein bilkul alag hain. Aksar log purani asli khabar ko naya bata ke share karte hain — asli outlet ke naam pe koi date check hi nahi karta. Group main clarification post kar dete hain, bilkul respectfully.",
    inspiredBy: {
      patternName: "Recirculated Real Article",
      country: "Global",
      year: "2015–ongoing",
      whatHappened: "Genuine articles from major outlets are shared years later as 'breaking' — no editing needed, just a fresh caption. Because the URL and outlet are real, most fact-checking instincts don't fire. It shapes public panic during elections, currency shocks, and health scares.",
      prevention: [
...

  {
    ...
    teaser: "A 20-second reel of rubble and crying children directs to a personal easypaisa — posted 4 hours after the quake.",
    tier: 3,
    verdict: "FALSE",
    format: "video",
    tacticId: "phishing",
    sender: { name: "Cousin Hina", relationship: "Instagram DM", voice: "emotional, tearful, first-responder-adjacent" },
    senderMotive: "She saw the reel go viral, cried, and forwarded before checking. She wants to be someone who helps immediately.",
    opener: "Api dekho — abhi zalzale ki footage. Account number description main hai. Please jitna ho sake donate karain, jaldi.",
    forward: {
    ...
    actions: [
      { id: "reverse-image", label: "Reverse-image a frame of the rubble", tool: "reverse_image", result: "The footage is from a 2023 earthquake in another country — reused. The current disaster is real; this specific video is not from it.", decisive: true },
      { id: "check-source", label: "Search the wallet number and 'Aslam'", tool: "check_source", result: "The number appears on FIA cybercrime alerts and on posts by real relief workers flagging it as a recurring disaster-scam number.", decisive: true },
      { id: "cross-check", label: "Compare with real relief orgs on the ground", tool: "cross_check", result: "Alkhidmat, Edhi, JDC and PRCS publish OFFICIAL relief accounts on their verified sites and handles. None match this personal wallet.", decisive: true },
      { id: "check-date", label: "Note when the reel was posted vs the disaster", tool: "check_date", result: "Reel posted 4 hours after the quake. Real registered NGOs take at least a day to publish channels and always route through their official pages first.", decisive: true },
    ],
    truthNote: "Video is the fastest-spreading disaster-scam format now. The footage may be from another disaster entirely, but grief overrides checking. Real relief goes through registered orgs — never a personal wallet flashed on a viral reel.",
    respectfulScript: "Hina yeh reel scam hai — footage kisi aur mulk ke 2023 zalzale ki hai, aur yeh number FIA ki cybercrime warnings pe hai. Aap ka jazba bara hai — Alkhidmat aur JDC ke official accounts main aap ko bhejti hoon, wahan aap ka donation asli families tak pahunche ga. Reel ko report kar dein, warna aur log fasein ge ❤️",
    inspiredBy: {
      patternName: "Disaster Relief Video Scam",
      country: "Global",
      year: "2010–ongoing",
      whatHappened: "Within hours of any earthquake, flood, or cyclone, viral short-video 'relief' posts use recycled rubble footage from unrelated disasters and route donations to a personal wallet. They out-race real NGOs by design.",
      prevention: [
  ...
  /* ── AI LITERACY — deliberately flawless. Spotting fails BY DESIGN. ── */
  {
    ...
    teaser: "A perfect 8-second voice note from your boss: \"Push through the vendor payment today.\" No artifacts. Nothing to spot.",
    tier: 3,
    verdict: "FALSE",
    format: "video",
    aiGenerated: true,
    tacticId: "forgery-engine",
    sender: { name: "'Boss' (unknown number)", relationship: "WhatsApp — new number", voice: "your actual boss's voice, exactly" },
    senderMotive: "The attacker scraped a minute of the CEO's public interviews and cloned the voice. They chose Friday 4:55pm on purpose.",
    opener: "Voice note from a new number claiming to be your boss. It sounds EXACTLY like him — because it is his voice, cloned.",
    forward: {
    ...
    actions: [
      { id: "listen-artifacts", label: "Listen closely for AI tells", tool: "reverse_image", result: "There are none. Rhythm is perfect. Breathing is natural. The little 'thanks' is exact. State-of-the-art voice cloning has already passed this test. Spotting cannot win here — do not rely on your ear.", decisive: false },
      { id: "check-source", label: "Check the sender number against the one you already have", tool: "check_source", result: "This is a NEW number. Your boss's real number is saved in your contacts and has not sent this. That single fact outweighs the voice.", decisive: true },
      { id: "call-back", label: "Call your boss BACK on his known number", tool: "check_source", result: "You reach him. He is at his kid's football match. He did not send anything. He asks you to forward the number to IT.", decisive: true },
      { id: "cross-check", label: "Cross-check with finance protocol", tool: "cross_check", result: "Company policy requires vendor payments to be approved via the finance portal — never over WhatsApp, never on voice alone. Protocol is the second independent check.", decisive: true },
    ],
    truthNote: "You could not have spotted this by listening. Nobody can anymore. You VERIFIED it — you called the real number, you checked the protocol. That is the skill the Forgery Engine forces us all to learn. 'Sounds real' is no longer proof of anything.",
    respectfulScript: "IT — flag this number. Someone has cloned [CEO]'s voice and used a new number to request a wire transfer to 'Zenith vendors'. I verified with him directly on his real number; he did not send this. Locking the vendor payment. Suggest a company-wide reminder that voice/video alone can no longer authorise anything.",
    inspiredBy: {
      patternName: "AI Voice-Clone CFO Fraud",
      country: "Global",
      year: "2019–ongoing",
      whatHappened: "Attackers have used AI-cloned voices of CEOs and CFOs — sometimes on live calls — to authorise wire transfers of hundreds of thousands to millions of dollars. In documented cases the victims report the voice was indistinguishable from the real person. The winning defence has been out-of-band verification and multi-party approval, never ear-based judgement.",
      prevention: [
...

  {
    ...
    teaser: "A 40-second clip of a familiar-looking news anchor reading breaking news. Nothing looks wrong. Nothing is real.",
    tier: 3,
    verdict: "FALSE",
    format: "video",
    aiGenerated: true,
    tacticId: "forgery-engine",
    sender: { name: "Cousin Waleed", relationship: "Family group", voice: "'saw it on the news' authority" },
    senderMotive: "He believes he's sharing a real breaking-news bulletin — the anchor and studio look completely normal.",
    ...
    actions: [
      { id: "spot-artifacts", label: "Study the clip frame-by-frame for AI tells", tool: "reverse_image", result: "There is nothing to spot. Lip sync is exact. Blink rate is normal. The lower-third font is right. Modern face-and-voice synthesis has closed this gap — do not rely on your eyes here.", decisive: false },
      { id: "check-channel", label: "Open the channel's real website and live stream", tool: "check_source", result: "The channel's real site and live stream carry no such bulletin. Their real anchors are on air discussing a completely different story right now.", decisive: true },
      { id: "cross-check", label: "Cross-check any other outlet", tool: "cross_check", result: "Zero corroboration on Reuters, AFP, Dawn, Al Jazeera, or the channel's own YouTube. A real policy reversal produces dozens of independent reports within minutes.", decisive: true },
      { id: "check-official", label: "Check the ministry's own site + social handles", tool: "check_source", result: "No announcement. Ministry spokesperson has posted a warning that a synthetic 'bulletin' is circulating using a familiar anchor's likeness.", decisive: true },
    ],
    truthNote: "You couldn't see it. Nobody can anymore. You VERIFIED it instead — you opened the channel yourself, you looked for a second witness, you checked the ministry directly. That is the whole skill now. Spotting is dying. Verifying is forever.",
    respectfulScript: "Waleed bhai — main ne channel ki live stream aur website khud kholi, iss bulletin ka koi wujood nahi. Kisi aur outlet ne bhi report nahi kiya, aur ministry ne khud warning issue ki hai ke anchor ki shakal use kar ke fake bulletin bana hai. Video AI-generated hai. Group main clarification bhej deta hoon so aur log na phailain.",
    inspiredBy: {
      patternName: "AI-Synthesized News Anchor",
      country: "Global",
      year: "2023–ongoing",
      whatHappened: "Fully synthetic 'news bulletins' impersonating real anchors and channel branding have been used to push fake political claims, market-moving rumours, and health hoaxes. As the models close the gap, spotting collapses — the defence is source and channel verification, not visual inspection.",
      prevention: [
