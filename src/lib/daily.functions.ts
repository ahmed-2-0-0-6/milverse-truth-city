...
export const fetchDailySplit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: rows, error } = await supabase
      .rpc("get_daily_split", { _drop_date: data.dropDate, _case_id: data.caseId });
...
export const fetchSharpestWatch = createServerFn({ method: "GET" })
  .handler(async () => {
    const supabase = serverClient();
...
export const fetchMostDeviousDesigner = createServerFn({ method: "GET" })
  .handler(async () => {
    const supabase = serverClient();
