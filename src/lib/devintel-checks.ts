...
export function runHeuristicChecks(): HeuristicResult[] {
  ...
  // ── FEED catalog integrity ────────────────────────────────────
  {
    ...
    const missingTruthNote = FEED_SCENARIOS.filter((s) => !s.truthNote || s.truthNote.length < 10).map((s) => s.id);
    const missingRespectful = FEED_SCENARIOS.filter((s) => !s.respectfulScript || s.respectfulScript.length < 10).map((s) => s.id);

    out.push({
      id: "feed-tactic",
      label: `Every Feed case has a tactic mapping (${total - missingTactic.length}/${total})`,
      detail: missingTactic.length === 0 ? "All feed cases mapped to a manual entry." : `Missing: ${missingTactic.slice(0, 5).join(", ")}${missingTactic.length > 5 ? "…" : ""}`,
    ...
    out.push({
      ...
      detail: missingTruthNote.length === 0 ? "All cases have a dossier ground-truth explanation." : `Missing: ${missingTruthNote.slice(0, 5).join(", ")}`,
    ...
    out.push({
      ...
      detail: missingRespectful.length === 0 ? "All cases include a Zubaida-safe reply line." : `Missing: ${missingRespectful.slice(0, 5).join(", ")}`,
    ...
    const orphans = FEED_SCENARIOS.filter((s) => s.tacticId && !manualIds.has(s.tacticId)).map((s) => s.id);
    out.push({
      id: "feed-tactic-resolves",
      label: "Feed tactic ids resolve to a Field Manual entry",
      detail: orphans.length === 0 ? "All tactic ids exist in the manual." : `Orphans: ${orphans.slice(0, 5).join(", ")}`,
  ...
  // ── MIRROR catalog integrity ──────────────────────────────────
  {
    ...
    out.push({
      ...
      detail: missingDossier.length === 0 ? "All mirror scenarios carry ground-truth facts." : `Missing dossier: ${missingDossier.slice(0, 5).join(", ")}`,
      status: missingDossier.length === 0 ? "pass" : "fail",
    });

    const missingPersona = mirror.filter((s) => !s.persona?.voice || !Array.isArray(s.persona.fillers)).map((s) => s.id);
    out.push({
      id: "mirror-persona",
      label: `Every Mirror scenario has a persona voice + fillers`,
      detail: missingPersona.length === 0 ? "Persona voice present on all." : `Missing: ${missingPersona.slice(0, 5).join(", ")}`,
  ...
  // ── Manual coverage ───────────────────────────────────────────
  {
    const missingCounter = MANUAL_ENTRIES.filter((e) => !e.counterMove || e.counterMove.length < 10).map((e) => e.id);
    out.push({
      id: "manual-counter",
      label: `Every manual entry has a counter-move`,
      detail: missingCounter.length === 0 ? "All entries teach a real defensive move." : `Missing: ${missingCounter.join(", ")}`,
  ...
  // ── Daily Drop rollover ───────────────────────────────────────
  {
    ...
    const okDate = /^\d{4}-\d{2}-\d{2}$/.test(today) && /^\d{4}-\d{2}-\d{2}$/.test(tomorrow) && tomorrow > today;
    out.push({
      id: "drop-rollover",
  ...
  // ── "coming soon" scan on shipped scenario catalog ────────────
  {
    ...
    out.push({
      ...
      detail: total === 0 ? "Clean." : `Feed: ${feedHits.join(", ")} · Mirror: ${mirrorHits.join(", ")}`,
