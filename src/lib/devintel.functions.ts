...
function summarize(rows: TmRow[]): IntelSummary {
    ...
    switch (r.event_type) {
      ...
      case "drop_play": {
        ...
        if (p.stake_bucket) {
          ...
          if (p.correct)
            wagerCorrectByBucket[p.stake_bucket] = (wagerCorrectByBucket[p.stake_bucket] ?? 0) + 1;
        }
        if (r.session_id) {
  ...
  const caseFunnel = Array.from(caseStarts.entries())
    .map(([case_id, starts]) => {
  ...
  const toolPicksArr = Array.from(toolPicks.entries())
    .map(([tool, b]) => ({
      tool,
      picks: b.picks,
  ...
  for (const r of KNOWN_ROUTES) {
    ...
    else if (c / totalRouteVisits < 0.005)
      deadZones.push({ route: r, reason: `${c} visits (<0.5% of traffic)` });
  }
...
function deterministicBrief(sum: IntelSummary): Brief {
  ...
  if (sum.mostAbandoned[0]) {
    ...
    items.push({
      ...
      hypothesis:
        "Difficulty spike, unclear dossier, or brief screen too long. Players quit before verdict.",
      prompt: wrapPrompt(
  ...
  if (sum.hardestCases[0]) {
    ...
    items.push({
      ...
      hypothesis:
        "Either the case is a great teaching moment or the ground-truth is misleading. Verify dossier is unambiguous.",
      prompt: wrapPrompt(
  ...
  if (sum.deadZones.length > 0) {
    ...
    items.push({
      ...
      evidence: z.reason,
      hypothesis:
        "Feature is either undiscoverable from nav, or shipped but not yet linked from key surfaces.",
      prompt: wrapPrompt(
  ...
  if (items.length === 0) {
    items.push({
      ...
      prompt: wrapPrompt(
        "Add a share prompt after Daily Drop completion so players invite one friend to try today's case.",
      ),
    });
  }
    ...
    if (key) {
      try {
        ...
        const compact = {
          ...
          shareCopies: sum.shareCopies,
          liteFallbacks: sum.liteFallbacks,
