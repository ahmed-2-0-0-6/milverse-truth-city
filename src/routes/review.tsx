...
export const Route = createFileRoute("/review")({
  head: () => ({
    meta: [{ title: "Review Queue — MILVERSE" }, { name: "robots", content: "noindex" }],
  }),
        ...
        {tab === "submissions" && (
          <>
            <div className="mb-4 flex items-center gap-3">
              ...
              <button
                onClick={refresh}
                className="rounded-sm border border-border px-3 py-1 stencil text-[10px] hover:bg-accent"
              >
                REFRESH
              </button>
            </div>
              ...
              {rows.map((r) => (
                <SubmissionCard
                  ...
                  onApprove={async (scenario) => {
                    ...
                    await approveFn({
                      data: { passcode, id: r.id, shareCode: code, scenario } as never,
                    });
                    setRows((cur) => cur.filter((x) => x.id !== r.id));
...
function AssessmentTab({ passcode }: { passcode: string }) {
  ...
  async function load(c: string) {
    ...
    try {
      const res = (await fetchFn({ data: { groupCode: upper } as never })) as unknown as {
        entries: CloudAssessmentRow[];
        phase: { phase: "intake" | "exit" };
      };
      setRows(res.entries ?? []);
  ...
  const attempts: CohortAttempt[] = useMemo(
    () =>
      rows.map((r) => ({
        codenameHash: r.codename_hash,
        phase: r.phase,
        form: r.form,
        items: r.items,
        metrics: {
          ...
          calibrationGap: r.calibration_gap,
          overconfidentErrors: r.overconfident_errors,
          missedScams: r.missed_scams,
          falseAlarms: r.false_alarms,
          unverifiableRecognized: r.unverifiable_recognized,
        },
  ...
  const daysSpan = useMemo(() => {
    ...
    const intakeTs = attempts
      .filter((a) => a.phase === "intake")
      .map((a) => a.ts)
      .sort();
    const exitTs = attempts
  ...
  function exportCsv() {
    ...
    for (const r of rows) {
      ...
      const cells: string[] = [
        ...
        r.created_at,
        String(r.accuracy),
        String(r.mean_confidence),
        String(r.calibration_gap),
        String(r.overconfident_errors),
        String(r.missed_scams),
        String(r.false_alarms),
    ...
    <section>
      <div className="rounded-sm border border-border bg-card p-5">
        ...
        <div className="mt-2 flex gap-2">
          ...
          {loadedCode && (
            <button
              onClick={() => load(loadedCode)}
              className="rounded-sm border border-border px-3 stencil text-[10px] hover:bg-accent inline-flex items-center gap-1"
            >
      ...
      {loadedCode && (
        <>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <span
              className={`stencil text-[10px] px-2 py-0.5 rounded-sm border ${phase === "exit" ? "border-caution text-caution" : "border-primary text-primary"}`}
            >
              {phase.toUpperCase()}
            </span>
            <div className="flex-1" />
          ...
          {headline && (
            <div className="mt-4 rounded-sm border-2 border-primary bg-primary/10 p-4">
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">
                Based on {rollup.nPaired} of {Math.max(rollup.nIntake, rollup.nExit)} student
                {Math.max(rollup.nIntake, rollup.nExit) === 1 ? "" : "s"} completing both phases.
              </p>
            </div>
          )}
          ...
          {rollup.nPaired < 5 ? (
            <div className="mt-4 rounded-sm border border-caution/40 bg-caution/5 p-4 text-sm">
              ...
              <p className="text-muted-foreground">
                Cohort metrics and per-tactic breakdown are shown when the group is larger (at least
                5 paired participants). Currently paired: {rollup.nPaired}. Day-level activity only
                — exact timestamps are not shown.
              </p>
            </div>
          ) : (
            <>
              {rollup.meanIntake && rollup.meanExit && (
                <div className="mt-4 rounded-sm border border-border bg-card p-5">
                  <div className="stencil text-[10px] text-muted-foreground">
                    COHORT MEANS · PAIRED PARTICIPANTS ONLY
                  </div>
                  <div className="mt-3 space-y-3">
                    <PairedBar
                      label="Accuracy (out of 6)"
                      pre={rollup.meanIntake.accuracy}
                      post={rollup.meanExit.accuracy}
                      max={6}
                      higherIsBetter
                    />
                    <PairedBar
                      label="Calibration gap (|value|)"
                      pre={Math.abs(rollup.meanIntake.calibrationGap)}
                      post={Math.abs(rollup.meanExit.calibrationGap)}
                      max={50}
                      higherIsBetter={false}
                      note={`${rollup.meanIntake.calibrationGap} → ${rollup.meanExit.calibrationGap}`}
                    />
                    <PairedBar
                      label="Overconfident errors"
                      pre={rollup.meanIntake.overconfidentErrors}
                      post={rollup.meanExit.overconfidentErrors}
                      max={6}
                      higherIsBetter={false}
                    />
                    <PairedBar
                      label="Missed scams"
                      pre={rollup.meanIntake.missedScams}
                      post={rollup.meanExit.missedScams}
                      max={2}
                      higherIsBetter={false}
                    />
                    <PairedBar
                      label="False alarms"
                      pre={rollup.meanIntake.falseAlarms}
                      post={rollup.meanExit.falseAlarms}
                      max={2}
                      higherIsBetter={false}
                    />
                  </div>
                ...
                <div className="mt-3 space-y-2">
                  {rollup.perTactic.map((t) => (
                    <div key={t.pairId} className="flex items-center gap-3 text-sm">
                      ...
                      <div
                        className={`w-16 text-right font-mono ${t.exitCorrectPct != null && t.intakeCorrectPct != null && t.exitCorrectPct > t.intakeCorrectPct ? "text-primary" : ""}`}
                      >
                        {t.exitCorrectPct ?? "—"}%
                      </div>
                    </div>
              ...
              <div className="mt-4 rounded-sm border border-border bg-card p-5">
                <div className="stencil text-[10px] text-muted-foreground">
                  PER-STUDENT · CODENAME ONLY · DAY-LEVEL
                </div>
                  ...
                  <tbody>
                    {rollup.perStudent.map((s) => (
                      <tr key={s.codenameHash} className="border-t border-border/60">
                        ...
                        <td className="py-1.5">
                          {s.intakeAt ? (
                            <CheckCircle2 className="inline h-3 w-3 text-primary" />
                          ) : (
          ...
          <p className="mt-4 text-[11px] text-muted-foreground leading-relaxed">
            <b>Anonymous by design:</b> codenames only, no accounts, no names. Two matched forms
            (A/B), counterbalanced by codename hash, so no item repeats between phases.
            Self-selected school sample; N is small; this is a pilot signal, not a peer-reviewed
            study.
          </p>
        </>
      )}
...
function Stat({
  ...
  return (
    <div
      className={`rounded-sm border p-3 ${accent ? "border-primary/50 bg-primary/5" : "border-border bg-card"}`}
    >
  ...
  return (
    <div>
      ...
      <div className="mt-1 space-y-1">
        ...
        <div className="h-2 rounded-sm bg-border/40">
          <div
            className={`h-2 rounded-sm ${improved ? "bg-primary" : "bg-caution/70"}`}
            style={{ width: `${postW}%` }}
          />
        </div>
      </div>
...
function SubmissionCard({
  ...
  const [contactClaim, setContactClaim] = useState(
  ...
  async function publish() {
    ...
    const scenario = {
      ...
      evidenceChips: [
        {
          id: "c1",
          label: "Rushed with urgency",
          correct: true,
          explain: "Urgency is a manipulation lever.",
        },
        {
          id: "c2",
          label: "Asked me to send money / codes",
          correct: true,
          explain: "The ask itself is the scam.",
        },
        {
          id: "c3",
          label: "Sounded polite",
          correct: false,
          explain: "Politeness is not evidence of legitimacy.",
        },
      ],
  ...
  return (
    <article className="rounded-sm border border-border bg-card p-5">
      <div className="flex items-center gap-2 stencil text-[9px] text-muted-foreground">
        ...
        <span>·</span>
        <span>
          {row.story.country}, {row.story.year}
        </span>
        <span>·</span>
      ...
      <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
        <div className="rounded-sm border border-border p-2">
          <b>Wanted:</b> {row.story.whatScammerWanted}
        </div>
        <div className="rounded-sm border border-border p-2">
          <b>Tell:</b> {row.story.whatTippedYouOff}
        </div>
      </div>
      ...
      {mode === "view" && (
        <div className="mt-4 flex gap-2">
          ...
          <button
            ...
            className="rounded-sm border border-destructive/60 bg-destructive/10 px-3 py-1.5 stencil text-[10px] text-destructive hover:bg-destructive/20"
          >
      ...
      {mode === "reject" && (
        <div className="mt-4 space-y-2">
          <textarea
            ...
            className="w-full min-h-[70px] rounded-sm border border-border bg-background p-2 text-sm"
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="rounded-sm border border-border px-3 py-1.5 stencil text-[10px] hover:bg-accent"
            >
              CANCEL
            </button>
          </div>
      ...
      {mode === "edit" && (
        <div className="mt-4 rounded-sm border border-primary/40 bg-primary/5 p-4 space-y-3">
          <div className="stencil text-[10px] text-primary">
            PRE-FILLED FROM STORY · EDIT BEFORE PUBLISH
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block">
              ...
              <select
                value={truth}
                onChange={(e) => setTruth(e.target.value as "REAL" | "IMPOSTER")}
                className="w-full rounded-sm border border-border bg-background p-2 text-sm"
              >
          ...
          <label className="block">
            ...
            <input
              value={contactClaim}
              onChange={(e) => setContactClaim(e.target.value)}
              className="w-full rounded-sm border border-border bg-background p-2 text-sm"
            />
          </label>
          <label className="block">
            <div className="text-xs text-muted-foreground">Opener</div>
            <textarea
              value={opener}
              onChange={(e) => setOpener(e.target.value)}
              className="w-full min-h-[80px] rounded-sm border border-border bg-background p-2 text-sm"
            />
          </label>
          <label className="block">
