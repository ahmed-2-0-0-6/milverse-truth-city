...
export const Route = createFileRoute("/drop")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "One case a day. Same for everyone. Wager Trust, verify, and post your receipt." },
      { property: "og:title", content: "AAJ KA FORWARD — MILVERSE" },
      { property: "og:description", content: "One case a day. One wager. One receipt. The city's daily verification ritual." },
    ],
  }),
  component: DropPage,
});
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="mt-4 flex items-center gap-3">
          <div className="relative h-8 w-8">
            {!status.playedToday && <span className="absolute inset-0 rounded-full bg-primary/40 animate-ping" />}
          ...
          <div>
            <div className="font-mono text-[10px] tracking-[0.3em] text-primary flex items-center gap-2">
              {friday && <span className="rounded-sm bg-primary/20 px-1.5 py-0.5">DESIGNER FRIDAY</span>}
              AAJ KA FORWARD · {today.dateKey}
            </div>
            <h1 className="mt-0.5 text-4xl sm:text-5xl font-black tracking-tight" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>
              TODAY'S FORWARD.
            </h1>
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 rounded-sm border border-border bg-card px-4 py-3">
          <div className="flex items-center gap-1.5 stencil text-[10px] text-primary">
            <Coins className="h-3.5 w-3.5" /> TRUST <span className="text-foreground tabular-nums">{status.trust}</span>
          </div>
          ...
          <div className="stencil text-[10px] text-muted-foreground">NEXT DROP {hh}:{mm}:{ss}</div>
        </div>

        {status.playedToday ? (
          <PostPlayState status={status} today={today} onOpenYesterday={() => setShowYesterday(true)} />
        ) : (
          <PlayFlow scenario={today.scenario} dateKey={today.dateKey} onDone={() => setStatus(readDailyStatus())} />
        )}
        ...
        <section className="mt-10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="stencil text-[10px] text-muted-foreground">THE CITY HAS MOVED ON.</span>
          </div>
          {showYesterday && (
            <div className="rounded-sm border border-border bg-card p-4 opacity-90">
              <div className="stencil text-[10px] text-muted-foreground">YESTERDAY · {yesterday.dateKey}</div>
              <div className="mt-1 text-lg font-black" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>{yesterday.scenario.title}</div>
              <div className="mt-2">
                <FormatFrame
              ...
              <div className="mt-3 text-xs text-muted-foreground">
                Answer: <span className="text-foreground stencil">{toDailyVerdict(yesterday.scenario.verdict)}</span>
                {yesterday.scenario.tacticId && <> · Tactic: <span className="text-foreground">{yesterday.scenario.tacticId}</span></>}
...
function PostPlayState({ status, today, onOpenYesterday }: {
  ...
  const receipt: ReceiptData = useMemo(() => ({
    ...
    siteUrl: typeof window !== "undefined" ? `${window.location.origin}/drop` : "milverse.app/drop",
  }), [entry, status.streak]);
  ...
  return (
    <div className="mt-6 space-y-6">
      <div className="rounded-sm border border-primary/40 bg-primary/[0.05] p-5">
        <div className="stencil text-[10px] tracking-widest text-primary mb-2">TODAY'S CALL — LOCKED</div>
        <div className="text-3xl font-black" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>
          {entry.correct ? "CORRECT CALL." : entry.truth === "SCAM" ? "MISSED SCAM." : "FALSE ALARM."}
        </div>
        <div className="mt-2 text-sm text-muted-foreground">
          You called <span className="text-foreground stencil">{entry.verdict}</span>.
          The city dossier says <span className="text-foreground stencil">{entry.truth}</span>.
          Stake {entry.stake} → <span className={entry.correct ? "text-primary" : "text-destructive"}>{entry.correct ? "+" : ""}{entry.delta}</span> Trust.
        </div>
        {pct !== null && (
          <div className="mt-4">
            ...
            <div className="mt-1 text-xs text-muted-foreground">{split!.total} watchers reported in.</div>
          </div>
        )}
        {today.scenario.tacticId && (
          <div className="mt-4 rounded-sm border border-border bg-background/50 p-3 text-sm">
            ...
            <div className="text-lg font-black mt-0.5">{today.scenario.tacticId.replace(/-/g, " ").toUpperCase()}</div>
            <div className="mt-1 text-xs text-muted-foreground">{today.scenario.truthNote}</div>
            <Link to="/manual/$entryId" params={{ entryId: today.scenario.tacticId }} className="mt-2 inline-flex items-center gap-1 stencil text-[10px] text-primary">
...
function PlayFlow({ scenario, dateKey, onDone }: { scenario: FeedScenario; dateKey: string; onDone: () => void }) {
  ...
  const runProbe = (kind: FeedToolKind) => {
    ...
    const evidence = scenario.toolbelt?.[kind]
  ...
  const lockVerdict = (v: "LEGIT" | "SCAM" | "MISLEADING") => { setVerdict(v); setStage("wager"); };
  ...
  const commit = async () => {
    ...
    if (!result) { setCommitting(false); onDone(); return; }
      ...
      {stage === "intake" && (
        <>
          <div className="rounded-sm border border-border bg-card p-4">
            ...
            <div className="mt-1 text-2xl font-black" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>{scenario.title}</div>
            <div className="mt-3">
              <FormatFrame
          ...
          <div className="rounded-sm border border-primary/40 bg-primary/[0.05] p-4">
            <div className="stencil text-[10px] tracking-widest text-primary mb-2">YOUR VERDICT</div>
            ...
            <div className="mt-2 text-[10px] text-muted-foreground">
              Probes used: {probesUsed.length} / 2 · You always judge — the site never declares truth for you.
            </div>
          </div>
        </>
      )}
      ...
      {stage === "verdict-cinema" && verdict && (
        <>
          <VerdictMoment
            ...
            caseId={scenario.id}
            stampLabel={truth}
            outcome={verdict === truth ? "correct" : (truth === "SCAM" ? "missed_scam" : "false_alarm")}
            onDone={() => { setShowTactic(!!scenario.tacticId); setStage("reveal"); }}
          />
...
function QuickProbes({ scenario, probesUsed, probeResults, onRun }: {
  ...
  return (
    <div className="rounded-sm border border-border bg-card p-4">
      <div className="flex items-center justify-between">
        ...
        <div className="stencil text-[10px] text-muted-foreground">DOSSIER · {(scenario.format ?? "whatsapp").toUpperCase()}</div>
      </div>
          ...
          return (
            <button
              ...
              className={`rounded-sm border px-2 py-2 stencil text-[10px] transition ${
                ...
                : disabled ? "border-border/40 text-muted-foreground/60 cursor-not-allowed"
      ...
      {probeResults.length > 0 && (
        <div className="mt-3 space-y-2">
          {probeResults.map((r) => (
            <div key={r.kind} className="rounded-sm border border-primary/30 bg-background/60 p-2 text-sm">
              <div className="stencil text-[9px] text-primary">{r.kind.replace(/_/g, " ").toUpperCase()}</div>
...
function Wager({ verdict, stake, bounds, onStake, onConfirm, onBack, committing }: {
  ...
  useEffect(() => { if (clamped !== stake) onStake(clamped); }, [clamped, stake, onStake]);
  ...
  return (
    <div className="rounded-sm border-2 border-primary/60 bg-gradient-to-b from-black/60 to-primary/5 p-5">
      ...
      <div className="mt-1 text-2xl sm:text-3xl font-black" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>
      ...
      <div className="mt-6">
        ...
        <div className="mt-2 flex items-center justify-between text-[10px] stencil text-muted-foreground">
          ...
          <span className="text-primary text-lg font-black" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>{clamped}</span>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <div className="rounded-sm border border-primary/40 bg-primary/[0.06] p-3">
          ...
          <div className="text-2xl font-black text-primary" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>+{winAmt}</div>
        </div>
        <div className="rounded-sm border border-destructive/40 bg-destructive/[0.06] p-3">
          ...
          <div className="text-2xl font-black text-destructive" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>{lossAmt}</div>
        </div>
      </div>

      <div className="mt-6 flex items-center gap-3">
        <button onClick={onBack} className="stencil text-[10px] text-muted-foreground hover:text-foreground">← back to verdict</button>
        <div className="flex-1" />
        <button
          onClick={onConfirm}
          disabled={committing}
      ...
      <div className="mt-2 text-[10px] text-muted-foreground">
        Wager is fictional Trust — no real money anywhere in MILVERSE. If Trust hits 0 the city fronts you 50.
      </div>
    </div>
  );
}
...
function Leaderboards() {
  ...
  const [devious, setDevious] = useState<Array<{ case_id: string; plays: number; fooled_pct: number }>>([]);
  ...
  return (
    <section className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="rounded-sm border border-border bg-card p-4">
        <div className="stencil text-[10px] text-primary flex items-center gap-1.5"><Trophy className="h-3 w-3" /> SHARPEST WATCH</div>
        <div className="text-xs text-muted-foreground mt-0.5">Top calibration · minimum 7 daily plays</div>
        {sharp.length === 0 ? (
          <div className="mt-3 text-xs text-muted-foreground italic">First 7-play watchers will appear here.</div>
        ) : (
      ...
      <div className="rounded-sm border border-border bg-card p-4">
        <div className="stencil text-[10px] text-primary flex items-center gap-1.5"><Trophy className="h-3 w-3" /> MOST DEVIOUS DESIGNER</div>
        <div className="text-xs text-muted-foreground mt-0.5">Community cases · highest fooled-% (moderated)</div>
        {devious.length === 0 ? (
          <div className="mt-3 text-xs text-muted-foreground italic">No community daily cases yet — Designer Friday builds this list.</div>
        ) : (
          <ol className="mt-3 space-y-1 text-sm">
            {devious.map((r, i) => (
              <li key={r.case_id} className="flex items-center gap-3">
                <span className="stencil text-[10px] text-muted-foreground w-6">#{i + 1}</span>
                <span className="font-mono text-xs text-foreground flex-1 truncate">{r.case_id}</span>
