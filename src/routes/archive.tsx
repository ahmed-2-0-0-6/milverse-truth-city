  ...
  head: () => ({
    meta: [
      { title: "The City Library — MILVERSE" },
      {
        name: "description",
        content:
          "The living memory of MILVERSE. Every case the city solves is archived here — written by its players.",
      },
    ],
  }),
        ...
        setCommunity(
          rows
            .map((r) => {
              try {
                const s = (
                  typeof r.scenario_config === "string"
                    ? JSON.parse(r.scenario_config)
                    : r.scenario_config
                ) as Scenario;
                return { ...r, scenario: s };
              } catch {
    ...
    <div className="min-h-screen grain">
      ...
      <main className="mx-auto max-w-6xl px-4 py-10">
        ...
        <div className="mb-4 flex flex-wrap gap-1 rounded-sm border border-border bg-card p-1 w-fit">
          <TabBtn
            active={shelf === "official"}
            onClick={() => setShelf("official")}
            icon={<ScrollText className="h-3 w-3" />}
            label={`Official (${officialAll.length})`}
          />
          <TabBtn
            active={shelf === "community"}
            onClick={() => setShelf("community")}
            icon={<Users className="h-3 w-3" />}
            label={`Community (${community.length})`}
          />
          <TabBtn
            active={shelf === "mine"}
            onClick={() => setShelf("mine")}
            icon={<BookMarked className="h-3 w-3" />}
            label={`My Shelf (${mineHistory.length})`}
          />
        </div>
        ...
        <div className="mb-6 flex flex-wrap items-center gap-2">
          ...
          <ChipGroup
            value={truth}
            onChange={(v) => setTruth(v as Truth)}
            options={[
        ...
        {shelf === "official" && (
          <ShelfSection
            title="MILVERSE OFFICIAL"
            tone="primary"
            note="New official missions released regularly — covering global and emerging information threats."
          >
            <SpineGrid>
              {officials.map((s) => (
        ...
        {shelf === "community" && (
          <ShelfSection
            title="FROM THE COMMUNITY · HUMAN-REVIEWED"
            tone="caution"
            note={
              <>
                <span className="inline-flex items-center gap-1.5">
                  <ShieldCheck className="h-4 w-4 text-primary" /> Every community mission is
          ...
          >
            ...
            <SpineGrid>
              {communityFiltered.map((row) => (
                ...
                >
                  <div className="folder-tab bg-caution/20 text-caution">
                    CASE · {row.share_code}
                  </div>
                  <div className="folder-body">
                    <div className="flex items-start justify-between gap-2">
                      <div className="text-sm font-semibold group-hover:text-caution">
                        {row.scenario.title}
                      </div>
                    ...
                    <div className="mt-3 flex items-center justify-between stencil text-[9px]">
                      <span className="text-caution/80">
                        T{row.scenario.tier} · {row.scenario.truth}
                      </span>
                      {row.scenario.designerRank ? (
                        <span className="text-primary/80">
                          BYLINE · {row.scenario.designerRank}
                        </span>
                      ) : (
                        row.scenario.inspiredBy && (
                          <span className="text-muted-foreground">
                            {row.scenario.inspiredBy.country} · {row.scenario.inspiredBy.year}
                          </span>
                        )
                      )}
                    </div>
        ...
        {shelf === "mine" && (
          <ShelfSection
            title="MY SHELF · PERSONAL LIBRARY"
            tone="primary"
            note="Cases you've closed. Click any stamp to reopen the file."
          >
  ...
  return (
    <section className="mb-14">
      <div className="mb-3 flex items-center gap-3">
        <span
          className={`inline-flex items-center gap-1.5 rounded-sm border ${cls} px-2.5 py-1 stencil text-[10px]`}
        >
          {title}
        </span>
...
function SpineCard({ scenario }: { scenario: Scenario }) {
  return (
    <Link
      to="/mirror/$caseId"
      params={{ caseId: scenario.id }}
      className="group folder-card hover:border-primary"
    >
      <div className="folder-tab bg-primary/15 text-primary">
        T{scenario.tier} · {scenario.truth === "IMPOSTER" ? "DECEPTION" : "LEGIT"}
      </div>
      <div className="folder-body">
        ...
        <div className="mt-3 stencil text-[9px] text-primary/80 opacity-0 group-hover:opacity-100 transition">
          OPEN CASE →
        </div>
      </div>
    </Link>
...
function ClosedFile({ entry, onOpen }: { entry: HistoryEntry; onOpen: () => void }) {
  ...
  const stampText =
    entry.result === "correct"
      ? "CLOSED · CORRECT CALL"
      : entry.result === "missed_scam"
        ? "CLOSED · MISSED SCAM"
        : entry.result === "false_alarm"
          ? "CLOSED · FALSE ALARM"
          : "CLOSED · LUCKY GUESS";
  const stampColor =
    entry.result === "correct"
      ? "text-primary border-primary"
      : entry.result === "lucky_guess"
        ? "text-caution border-caution"
        : "text-destructive border-destructive";
  return (
    <button onClick={onOpen} className="group folder-card text-left hover:border-primary">
      <div className="folder-tab bg-primary/15 text-primary">
        T{entry.tier} · YOUR VERDICT: {entry.verdict}
      </div>
      <div className="folder-body relative">
        <div
          className={`absolute right-2 top-0 rotate-6 border-2 ${stampColor} bg-background/70 px-2 py-0.5 stencil text-[9px] tracking-widest`}
        >
          <span className="inline-flex items-center gap-1">
            <Stamp className="h-3 w-3" /> {stampText}
          </span>
        </div>
        <div className="text-sm font-semibold group-hover:text-primary pr-16">{title}</div>
        <div className="mt-1 text-xs text-muted-foreground line-clamp-2">
          {scenario?.teaser ?? "Case file from your city record."}
        </div>
        <div className="mt-3 stencil text-[9px] text-muted-foreground">
          TRUTH · {entry.truth} · {entry.points >= 0 ? "+" : ""}
          {entry.points} PTS
        </div>
      </div>
    </button>
  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full max-w-lg rounded-sm border-2 border-primary/60 bg-card p-6"
      >
        <button
          onClick={onClose}
        ...
        <div className="mt-2 stencil text-[10px] text-muted-foreground">
          T{entry.tier} · YOUR VERDICT: {entry.verdict} · TRUTH: {entry.truth} ·{" "}
        ...
        {scenario?.dossier?.knownFacts?.length ? (
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            Case details are no longer in the local library.
          </p>
        )}
