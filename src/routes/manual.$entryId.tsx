...
export const Route = createFileRoute("/manual/$entryId")({
  ...
  head: ({ loaderData }) => {
    ...
    return { meta: [{ title }, { name: "description", content: e?.oneLine ?? "MIL codex entry." }] };
  },
  component: EntryPage,
  notFoundComponent: () => (
    <div className="min-h-screen grain">
      ...
      <main className="mx-auto max-w-2xl px-4 py-16 text-center">
        ...
        <Link to="/manual" className="mt-4 inline-block stencil text-xs text-primary">← BACK TO MANUAL</Link>
      </main>
    </div>
  ),
  errorComponent: () => (
  ...
  return (
    <div className="min-h-screen grain">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        <Link to="/manual" className="inline-flex items-center gap-1 font-mono text-xs tracking-widest text-muted-foreground hover:text-foreground">
        ...
        <div className="mt-6 rounded-xl border-2 border-primary/40 bg-card p-6 sm:p-8 relative overflow-hidden">
          <div
            className="absolute -right-6 -top-3 rotate-12 stencil text-[9px] tracking-[0.35em] text-primary/60 border border-primary/40 px-3 py-1"
          >
            DECLASSIFIED · {e.code}
          </div>
          ...
          <section className="mt-8">
            <div className="stencil text-[10px] tracking-widest text-muted-foreground mb-2">HOW IT WORKS</div>
            <p className="text-sm leading-relaxed">{e.howItWorks}</p>
          </section>

          <section className="mt-6">
            <div className="stencil text-[10px] tracking-widest text-muted-foreground mb-2">HOW IT SHOWS UP · WORLDWIDE</div>
          ...
          <section className="mt-6">
            ...
            <ul className="space-y-1.5">
              {e.redFlags.map((r, i) => (
                <li key={i} className="text-sm flex gap-2"><span className="text-caution">·</span>{r}</li>
              ))}
            </ul>
          </section>
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            <Link to="/manual" className="stencil text-[10px] tracking-widest text-muted-foreground hover:text-foreground">← ALL FILES</Link>
            <span className="text-muted-foreground">·</span>
            <Link to="/manual/take-it-outside" className="inline-flex items-center gap-1 stencil text-[10px] tracking-widest text-primary">
