...
export const Route = createFileRoute("/pressroom")({
  head: () => ({
    meta: [{ title: "Pressroom — MILVERSE" }, { name: "robots", content: "noindex" }],
  }),
      ...
      <div className="min-h-screen grain">
        ...
        <main className="mx-auto max-w-md px-4 py-16">
          <div className="rounded-sm border border-border bg-card p-6">
            <div className="flex items-center gap-2 stencil text-xs text-primary">
              <Lock className="h-4 w-4" /> PRESSROOM · TEAM ONLY
            </div>
            <h1
              className="mt-2 text-3xl font-black"
              style={{ fontFamily: '"Bebas Neue", sans-serif' }}
            >
              THE DAILY MIRAGE — PRESSROOM
            </h1>
            <input
              type="password"
              value={passcode}
              ...
              className="mt-4 w-full rounded-sm bg-background border border-border px-3 py-2"
          ...
          <div className="space-y-1">
            {rows.map((r) => (
              <button
                ...
                onClick={() => open(r.edition_number)}
                className={`w-full text-left rounded-sm border px-3 py-2 ${selected === r.edition_number ? "border-primary bg-primary/10" : "border-border hover:border-primary/50"}`}
              >
                <div className="stencil text-[10px] text-primary">
                  #{String(r.edition_number).padStart(3, "0")} · {r.status.toUpperCase()}
                </div>
            ...
            <button
              onClick={() => {
                const next = (rows[0]?.edition_number ?? 0) + 1;
                setSelected(next);
              ...
              className="w-full text-left rounded-sm border border-dashed border-primary/40 px-3 py-2 stencil text-[10px] text-primary hover:bg-primary/5"
            >
              + NEW EDITION
            </button>
          </div>
          ...
          <div>
            {!content ? (
              <p className="text-sm text-muted-foreground">
                Pick an edition on the left, or start a new one.
              </p>
            ) : (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <input
                    ...
                    className="rounded-sm bg-background border border-border px-2 py-1 text-sm"
                  />
                  ...
                  <button
                    ...
                    className="rounded-sm border border-border px-3 py-1.5 stencil text-[10px] inline-flex items-center gap-1"
                  >
                  ...
                  <button
                    ...
                    className="rounded-sm border border-primary/60 bg-primary/10 text-primary px-3 py-1.5 stencil text-[10px] inline-flex items-center gap-1 disabled:opacity-40"
                  >
                ...
                {preview ? (
                  <div className="paper rounded-sm border border-border p-6">
                    <PaperFrontPage
                      lead={content.lead}
                      editionNumber={selected!}
                      editionDate={date}
                      onDone={() => {}}
                    />
                    <div className="paper-ornament my-6">PREVIEW</div>
                    <PaperForgeryColumn
                      forgery={content.forgery}
                      editionNumber={selected!}
                      onDone={() => {}}
                    />
                    <PaperSocialPage
...
function Editor({
  ...
  setContent: (c: EditionContent) => void;
}) {
  function up<K extends keyof EditionContent>(k: K, v: EditionContent[K]) {
  ...
  return (
    <div className="space-y-6">
      <Section title="A · LEAD (Front Page)">
        <Field label="Kicker">
          <Input v={content.lead.kicker} on={(v) => up("lead", { ...content.lead, kicker: v })} />
        </Field>
        <Field label="Headline">
          <Input
            v={content.lead.headline}
            on={(v) => up("lead", { ...content.lead, headline: v })}
          />
        </Field>
        <Field label="Subhead">
          <Input v={content.lead.subhead} on={(v) => up("lead", { ...content.lead, subhead: v })} />
        </Field>
        <Field label="Byline">
          <Input v={content.lead.byline} on={(v) => up("lead", { ...content.lead, byline: v })} />
        </Field>
        <Field label="Case ID (from FEED_SCENARIOS)">
          <Input v={content.lead.caseId} on={(v) => up("lead", { ...content.lead, caseId: v })} />
        </Field>
        <Field label="Columns (paragraphs, one per line)">
          <Textarea
            v={content.lead.columns.join("\n")}
            rows={5}
            on={(v) => up("lead", { ...content.lead, columns: v.split("\n").filter(Boolean) })}
          />
        </Field>
      </Section>
      ...
      <Section title="B · FORGERY">
        <Field label="Prompt">
          <Input
            v={content.forgery.prompt}
            on={(v) => up("forgery", { ...content.forgery, prompt: v })}
          />
        </Field>
        <Field label="Image alt">
          <Input
            v={content.forgery.imageAlt}
            on={(v) => up("forgery", { ...content.forgery, imageAlt: v })}
          />
        </Field>
        <Field label="Image emoji">
          <Input
            v={content.forgery.imageEmoji}
            on={(v) => up("forgery", { ...content.forgery, imageEmoji: v })}
          />
        </Field>
        <Field label="Truth">
          <select
            value={content.forgery.truth}
            onChange={(e) =>
              up("forgery", { ...content.forgery, truth: e.target.value as "REAL" | "AI" })
            }
        ...
        <Field label="Provenance">
          <Textarea
            v={content.forgery.provenance}
            on={(v) => up("forgery", { ...content.forgery, provenance: v })}
          />
        </Field>
        <Field label="Thesis">
          <Input
            v={content.forgery.thesis}
            on={(v) => up("forgery", { ...content.forgery, thesis: v })}
          />
        </Field>
        <Field label="Tactic">
          <TacticSelect
            v={content.forgery.tacticId}
            on={(v) => up("forgery", { ...content.forgery, tacticId: v })}
          />
        </Field>
      </Section>
      ...
      <Section title="C · SOCIAL PAGE">
        <Field label="Handle">
          <Input
            v={content.social.handle}
            on={(v) => up("social", { ...content.social, handle: v })}
          />
        </Field>
        <Field label="Caption">
          <Textarea
            v={content.social.caption}
            on={(v) => up("social", { ...content.social, caption: v })}
          />
        </Field>
        <Field label="Likes">
          <Input
            type="number"
            v={String(content.social.likes)}
            on={(v) => up("social", { ...content.social, likes: Number(v) })}
          />
        </Field>
        <Field label="Views">
          <Input
            type="number"
            v={String(content.social.views)}
            on={(v) => up("social", { ...content.social, views: Number(v) })}
          />
        </Field>
        <Field label="Image emoji">
          <Input
            v={content.social.imageEmoji}
            on={(v) => up("social", { ...content.social, imageEmoji: v })}
          />
        </Field>
        <Field label="Truth">
          <select
            value={content.social.truth}
            onChange={(e) =>
              up("social", {
                ...content.social,
                truth: e.target.value as "TRUE" | "FALSE" | "MISLEADING",
              })
            }
            className="bg-background border border-border rounded-sm px-2 py-1 text-sm"
          >
        ...
        <Field label="Reveal">
          <Textarea
            v={content.social.reveal}
            on={(v) => up("social", { ...content.social, reveal: v })}
          />
        </Field>
        <Field label="Tactic">
          <TacticSelect
            v={content.social.tacticId}
            on={(v) => up("social", { ...content.social, tacticId: v })}
          />
        </Field>
      </Section>
      ...
      <Section title="D · CLASSIFIEDS (3–4)">
          ...
          <div key={i} className="border border-border rounded-sm p-3 space-y-2">
            ...
            <Field label="Title">
              <Input
                v={c.title}
                on={(v) => {
                  const arr = content.classifieds.slice();
                  arr[i] = { ...c, title: v };
                  up("classifieds", arr);
                }}
              />
            ...
            <Field label="Flags (exact substrings, comma-separated)">
              <Input
                v={c.flags.join(", ")}
                on={(v) => {
                  const arr = content.classifieds.slice();
            ...
            <button
              onClick={() => {
                const arr = content.classifieds.filter((_, j) => j !== i);
                up("classifieds", arr);
              }}
              className="stencil text-[10px] text-destructive underline"
            >
              remove
            </button>
          </div>
        ...
        <button
          onClick={() =>
            up("classifieds", [
              ...content.classifieds,
              { title: "", body: "", flags: [], tacticId: "phishing" },
            ])
          }
          className="stencil text-[10px] text-primary underline"
        >
          + add classified
        </button>
      </Section>
      ...
      <Section title="E · PUZZLE">
        <Field label="Kind">
          <select
            value={content.puzzle.kind}
            onChange={(e) =>
              up("puzzle", {
                ...content.puzzle,
                kind: e.target.value as "headline_autopsy" | "spot_the_tell",
              })
            }
            className="bg-background border border-border rounded-sm px-2 py-1 text-sm"
          >
        ...
        <Field label="Clickbait line">
          <Input
            v={content.puzzle.clickbait}
            on={(v) => up("puzzle", { ...content.puzzle, clickbait: v })}
          />
        </Field>
        <Field label="Honest version">
          <Input
            v={content.puzzle.honest}
            on={(v) =>
              up("puzzle", { ...content.puzzle, honest: v, words: v.split(/\s+/).filter(Boolean) })
            }
          />
        </Field>
        <Field label="Reveal note">
          <Textarea
            v={content.puzzle.reveal}
            on={(v) => up("puzzle", { ...content.puzzle, reveal: v })}
          />
        </Field>
      </Section>

      <Section title="F · LEDGER">
        <Field label="Note (italic caption under stats)">
          <Input v={content.ledger.note ?? ""} on={(v) => up("ledger", { note: v })} />
        </Field>
      </Section>

      <Section title="G · EDITORIAL (Handler fallback)">
        <Field label="Fallback (used when AI slow/quiet)">
          <Textarea
            v={content.editorial.fallback}
            rows={4}
            on={(v) => up("editorial", { ...content.editorial, fallback: v })}
          />
        </Field>
        <Field label="Signoff">
          <Input
            v={content.editorial.signoff}
            on={(v) => up("editorial", { ...content.editorial, signoff: v })}
          />
        </Field>
      </Section>

      <Section title="H · REAL WORLD">
        <Field label="Lede (2 sentences)">
          <Textarea
            v={content.realWorld.lede}
            on={(v) => up("realWorld", { ...content.realWorld, lede: v })}
          />
        </Field>
        <Field label="Link label">
          <Input
            v={content.realWorld.linkLabel}
            on={(v) => up("realWorld", { ...content.realWorld, linkLabel: v })}
          />
        </Field>
        <Field label="Link URL">
          <Input
            v={content.realWorld.linkHref}
            on={(v) => up("realWorld", { ...content.realWorld, linkHref: v })}
          />
        </Field>
      </Section>
    </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <details open className="rounded-sm border border-border p-4">
      <summary className="cursor-pointer stencil text-xs text-primary">{title}</summary>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <div className="stencil text-[10px] text-muted-foreground mb-1">{label}</div>
      {children}
    </label>
  );
}
function Input({ v, on, type = "text" }: { v: string; on: (v: string) => void; type?: string }) {
  return (
    <input
      ...
      className="w-full rounded-sm bg-background border border-border px-2 py-1.5 text-sm"
    />
  );
}
function Textarea({ v, on, rows = 3 }: { v: string; on: (v: string) => void; rows?: number }) {
  return (
    <textarea
      ...
      rows={rows}
      onChange={(e) => on(e.target.value)}
      className="w-full rounded-sm bg-background border border-border px-2 py-1.5 text-sm font-mono"
    />
  );
}
