...
function DevIntelPage() {
  ...
  const [briefs, setBriefs] = useState<Array<{ id: string; created_at: string; brief: Brief; source: string }>>([]);
    ...
    return (
      <div className="min-h-screen grain">
        ...
        <main className="mx-auto max-w-md px-4 py-24 text-center">
          ...
          <h1 className="mt-3 text-2xl font-black" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>
            INTELLIGENCE DESK
          </h1>
          <p className="stencil text-[10px] text-muted-foreground mt-1">RESTRICTED · PASSCODE REQUIRED</p>
          <form
            onSubmit={(e) => { e.preventDefault(); void authenticate(); }}
          ...
          >
            ...
            />
            <button disabled={busy || !passcode} className="rounded-md bg-primary px-4 py-2 stencil text-[10px] tracking-widest text-primary-foreground disabled:opacity-50">
          ...
          <p className="mt-6 text-[10px] text-muted-foreground italic">
            Zero telemetry contains message content or player identity — only anonymous, aggregate counts.
          </p>
        </main>
      </div>
    );
    ...
    <div className="min-h-screen grain">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        <div className="flex items-baseline justify-between">
          <div>
            <div className="stencil text-[10px] tracking-[0.35em] text-primary">INTELLIGENCE DESK</div>
            <h1 className="text-4xl font-black" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>PRECINCT BOARD</h1>
          </div>
          <button onClick={refreshIntel} disabled={busy} className="stencil text-[10px] text-muted-foreground hover:text-foreground">REFRESH</button>
        </div>
...
function BoardTab({ intel, onRunBrief, busy }: { intel: IntelSummary; onRunBrief: () => void; busy: boolean }) {
  ...
  return (
    <div className="mt-5 grid gap-4 md:grid-cols-2">
      <Card title="TRAFFIC" chip={`${intel.windowDays}d WINDOW`}>
        ...
        <FunnelRow label="LANDING" count={intel.funnel.landing} base={intel.funnel.landing} pct={100} />
        <FunnelRow label="FIRST CASE" count={intel.funnel.firstCase} base={intel.funnel.landing} pct={funnelPct(intel.funnel.firstCase, intel.funnel.landing)} />
        <FunnelRow label="DAY-2 RETURN" count={intel.funnel.day2Return} base={intel.funnel.landing} pct={funnelPct(intel.funnel.day2Return, intel.funnel.landing)} />
      </Card>

      <Card title="TOP ROUTES">
        <List rows={intel.routeVisits.slice(0, 10).map((r) => ({ left: r.route, right: String(r.count) }))} />
        {intel.deadZones.length > 0 && (
          <>
            <div className="mt-4 stencil text-[10px] text-caution">DEAD ZONES</div>
            <List tone="warn" rows={intel.deadZones.map((z) => ({ left: z.route, right: z.reason }))} />
          </>
        )}
      </Card>

      <Card title="5 HARDEST CASES" chip="FOOL RATE">
        <List rows={intel.hardestCases.map((c) => ({
          left: c.case_id,
      ...
      <Card title="5 MOST ABANDONED" chip="ABANDON RATE">
        <List rows={intel.mostAbandoned.map((c) => ({
          left: c.case_id,
              ...
              return (
                <div key={bucket} className="flex items-center gap-2 text-xs">
                  ...
                  <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                    <div className={`h-full ${pct >= 60 ? "bg-primary" : "bg-caution"}`} style={{ width: `${pct}%` }} />
                  </div>
                  <span className="w-24 text-right text-muted-foreground font-mono">{pct}% · {count}</span>
                </div>
              );
            })}
          </div>
            ...
            {intel.dropRetention.map((d) => {
              ...
              return (
                <div key={d.day} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full bg-primary/70 rounded-t" style={{ height: `${(d.players / max) * 88}px` }} title={`${d.day}: ${d.players}`} />
      ...
      <Card title="TOOLBELT PICKS">
        <List rows={intel.toolPicks.map((t) => ({
          left: t.tool,
      ...
      <Card title="MANUAL OPENS">
        <List rows={intel.manualOpens.slice(0, 10).map((m) => ({ left: m.entry, right: String(m.count) }))} />
      </Card>

      <Card title="MISC">
        <div className="grid grid-cols-2 gap-3">
          <Stat label="SHARE COPIES" value={intel.shareCopies} />
          <Stat label="LITE FALLBACKS" value={intel.liteFallbacks} tone={intel.liteFallbacks > 10 ? "warn" : undefined} />
        </div>
      </Card>
            ...
            <ul className="space-y-1 text-[11px] font-mono">
              {intel.errors.slice(0, 8).map((e) => (
                <li key={e.message} className="text-destructive">
                  ...
                  {e.sample_route && <span className="text-muted-foreground"> · {e.sample_route}</span>}
                </li>
              ))}
            </ul>
          )}
      ...
      <div className="md:col-span-2 flex items-center justify-between rounded-md border border-primary/40 bg-primary/[0.06] p-4">
        <div>
          ...
          <div className="text-xs text-muted-foreground mt-1">Sends AGGREGATE stats (numbers only) to the AI. Never runs automatically.</div>
        </div>
        <button onClick={onRunBrief} disabled={busy} className="rounded-md bg-primary px-4 py-2 stencil text-[10px] tracking-widest text-primary-foreground disabled:opacity-50">
          {busy ? "ANALYZING…" : "RUN"}
        </button>
      </div>
    </div>
...
function BriefsTab({ briefs, onRunBrief, busy }: { briefs: Array<{ id: string; created_at: string; brief: Brief; source: string }>; onRunBrief: () => void; busy: boolean }) {
  return (
    <div className="mt-5 space-y-4">
      <div className="flex items-center justify-between rounded-md border border-primary/30 bg-card p-3">
        <div className="text-xs text-muted-foreground">Last {briefs.length}/10 briefs. Each cached with its snapshot.</div>
        <button onClick={onRunBrief} disabled={busy} className="rounded-md bg-primary px-3 py-1.5 stencil text-[10px] tracking-widest text-primary-foreground disabled:opacity-50">
          {busy ? "…" : "NEW BRIEF"}
        </button>
      </div>
      {briefs.length === 0 && <div className="text-sm text-muted-foreground">No briefs yet. Trigger one from the Precinct Board.</div>}
      {briefs.map((b) => (
        <div key={b.id} className="rounded-lg border-2 border-primary/30 bg-card p-4">
          <div className="flex items-baseline justify-between">
            <div className="stencil text-[10px] tracking-widest text-primary">{b.source === "ai" ? "AI BRIEF" : "FALLBACK BRIEF"}</div>
          ...
          <div className="mt-3 space-y-3">
            {b.brief.items.map((it, i) => (
              <div key={i} className="rounded-md border border-border bg-background/60 p-3">
                ...
                <div className="mt-1 text-xs text-muted-foreground"><span className="stencil text-[9px] text-primary">EVIDENCE · </span>{it.evidence}</div>
                <div className="mt-1 text-xs text-muted-foreground"><span className="stencil text-[9px] text-primary">HYPOTHESIS · </span>{it.hypothesis}</div>
                <div className="mt-3 relative">
                  <pre className="max-h-40 overflow-auto rounded-sm border border-border bg-black/30 p-2 text-[11px] whitespace-pre-wrap font-mono">{it.prompt}</pre>
                  <button
...
function ChecklistTab() {
  ...
  return (
    <div className="mt-5 space-y-4">
      <div className={`rounded-lg border-2 p-5 ${clear ? "border-primary/50 bg-primary/[0.06]" : "border-caution/50 bg-caution/[0.05]"}`}>
        <div className="stencil text-[10px] tracking-widest text-muted-foreground">PRE-FLIGHT</div>
        <div className="mt-1 text-4xl font-black" style={{ fontFamily: '"Bebas Neue", sans-serif' }}>
          {clear ? "CLEAR FOR JUDGES" : "HOLD"} · {pass}/{total}
        </div>
        <div className="text-xs text-muted-foreground mt-1">Deterministic checks. No AI, no network.</div>
      </div>
      <ul className="space-y-2">
        {results.map((r) => (
          <li key={r.id} className={`flex items-start gap-3 rounded-md border p-3 ${r.status === "pass" ? "border-border bg-card" : r.status === "warn" ? "border-caution/40 bg-caution/[0.06]" : "border-destructive/40 bg-destructive/[0.06]"}`}>
            {r.status === "pass" ? <CheckCircle2 className="h-4 w-4 text-primary mt-0.5" /> :
             r.status === "warn" ? <AlertTriangle className="h-4 w-4 text-caution mt-0.5" /> :
...
function Stat({ label, value, tone }: { label: string; value: number; tone?: "bad" | "warn" }) {
  const color = tone === "bad" ? "text-destructive" : tone === "warn" ? "text-caution" : "text-foreground";
  return (
    <div className="rounded-sm border border-border bg-background/40 p-2 text-center">
      <div className={`text-xl font-black ${color}`} style={{ fontFamily: '"Bebas Neue", sans-serif' }}>{value}</div>
      <div className="stencil text-[9px] text-muted-foreground">{label}</div>
    </div>
  );
}

function FunnelRow({ label, count, base, pct }: { label: string; count: number; base: number; pct: number }) {
  return (
    <div className="mt-2 flex items-center gap-2 text-xs">
      ...
      <span className="w-24 text-right font-mono text-muted-foreground">{count} · {pct}%</span>
    </div>
  );
}

function List({ rows, tone }: { rows: Array<{ left: string; right: string; tone?: "bad" | "warn" }>; tone?: "warn" }) {
    ...
    <ul className="space-y-1 text-xs">
      {rows.map((r, i) => (
        <li key={i} className="flex items-center justify-between gap-3 font-mono">
          <span className={`truncate ${tone === "warn" || r.tone === "warn" ? "text-caution" : "text-foreground"}`}>{r.left}</span>
          <span className={`shrink-0 ${r.tone === "bad" ? "text-destructive" : "text-muted-foreground"}`}>{r.right}</span>
        </li>
      ))}
    </ul>
  );
