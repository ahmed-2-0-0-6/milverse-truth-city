...
export function PaperLedger({ editionNumber, note }: { editionNumber: number; note?: string }) {
  ...
  useEffect(() => {
    void gp({ data: { number: editionNumber, section: "lead" } })
      .then((s) => setLead(s as { total: number; correct: number }))
      .catch(() => {});
    void gp({ data: { number: editionNumber, section: "forgery" } })
      .then((s) => setForg(s as { total: number; correct: number }))
      .catch(() => {});
    void fetchSharpestWatch()
      .then((r) => setSharp((r.rows ?? []).slice(0, 3)))
      .catch(() => {});
  }, [gp, editionNumber]);

  const leadPct =
    lead && lead.total ? Math.round(((lead.total - lead.correct) / lead.total) * 100) : null;
  ...
  return (
    <section className="mt-2">
      ...
      <div className="mt-2 border-y-4 double" style={{ borderColor: "var(--paper-ink)" }}>
        ...
        >
          <Cell
            label="FOOLED BY LEAD"
            value={leadPct === null ? "—" : `${leadPct}%`}
            sub={lead ? `${lead.total} plays` : ""}
          />
          <Cell
            label="FORGERY ACC."
            value={forgPct === null ? "—" : `${forgPct}%`}
            sub={forg ? `${forg.total} guesses` : ""}
          />
          <Cell
            label="SHARPEST"
            value={sharp[0]?.handle ?? "—"}
            sub={sharp[0] ? `${sharp[0].correct_pct}%` : ""}
          />
          <Cell label="EDITION" value={`#${String(editionNumber).padStart(3, "0")}`} sub="live" />
        </div>
      </div>
      {note && (
        <p className="paper-serif italic text-sm mt-3 text-[color:var(--paper-muted)]">{note}</p>
      )}
    </section>
  );
}
...
function Cell({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="p-3">
      ...
      <div className="paper-serif text-2xl mt-0.5" style={{ fontWeight: 900 }}>
        {value}
      </div>
      {sub && (
        <div className="paper-mono text-[10px] mt-0.5 text-[color:var(--paper-muted)]">{sub}</div>
      )}
    </div>
  );
}
