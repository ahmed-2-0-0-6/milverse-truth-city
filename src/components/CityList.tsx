...
export function CityList({ onSwitchToMap }: { onSwitchToMap: () => void }) {
  ...
  const recIds = useMemo(() => new Set(getMirrorRecommendations(profile).map((r) => r.scenario.id)), [profile]);
    ...
    <div className="rounded-sm border border-border/60 bg-card">
      <div className="flex items-center justify-between border-b border-border px-3 py-2">
        <div className="flex items-center gap-1 rounded-sm border border-primary/40 px-1 py-0.5 stencil text-[10px]">
          <button onClick={onSwitchToMap} className="px-2 py-1 rounded-sm text-muted-foreground hover:text-foreground">MAP</button>
...
function DistrictSection({
  ...
  color: string; Icon: typeof Eye; title: string; subtitle: string;
  done: number; total: number; stations: Station[]; played: Set<string>; recIds: Set<string>;
  ...
  return (
    <section className="border-b border-border last:border-b-0">
      <header className="flex items-center gap-3 p-3">
        <div className="flex h-8 w-8 items-center justify-center rounded-sm border" style={{ borderColor: color, color }}>
          <Icon className="h-4 w-4" />
        </div>
        <div className="flex-1">
          <h3 className="stencil text-sm" style={{ color }}>{title}</h3>
          <p className="text-xs text-muted-foreground">{subtitle}</p>
        </div>
        <div className="stencil text-[10px] text-muted-foreground">{done}/{total}</div>
      </header>
        ...
        {stations.map((s) => {
          ...
          return (
            <li key={s.scenario.id}>
              ...
              >
                <span className="stencil text-[10px] w-14 shrink-0" style={{ color }}>{s.code}</span>
                ...
                {isBeacon && <span className="stencil text-[9px] px-1.5 py-0.5 rounded-sm" style={{ background: `${color}22`, color }}>PLAN</span>}
                {isPlayed && <span className="stencil text-[9px] text-primary shrink-0">✓ DONE</span>}
