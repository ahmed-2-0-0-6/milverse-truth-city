  ...
  return (
    <div className="mt-6">
      <div className="mb-4 flex items-center justify-between">
        <div className="font-mono text-[11px] tracking-widest text-primary">THE PATH · {done} / {TOTAL_LESSONS}</div>
          ...
          {LESSONS.map((l) => {
            ...
            return (
              <li key={l.n} className="relative pl-16">
                <div className={`absolute left-2 top-3 flex h-8 w-8 items-center justify-center rounded-full border-2 ${
                  complete ? "border-primary bg-primary text-primary-foreground"
                  : unlocked ? "border-primary bg-background text-primary"
                  : "border-border bg-muted text-muted-foreground"
                }`}>
                  {complete ? <Check className="h-4 w-4" /> : unlocked ? <Circle className="h-3 w-3 fill-current" /> : <Lock className="h-3.5 w-3.5" />}
                </div>
                <button
                ...
                >
                  <div className="font-mono text-[10px] tracking-widest text-muted-foreground">LESSON {l.n}</div>
                  <div className="mt-1 font-semibold">{l.title}</div>
                  {!unlocked && <div className="mt-1 text-xs text-muted-foreground">Finish lesson {l.n - 1} to unlock.</div>}
                </button>
              </li>
            );
          })}
...
function LessonView({ lesson, onComplete }: { lesson: Lesson; onComplete: (caseId: string) => void }) {
    ...
    <div className="mt-4 space-y-5">
      <div className="rounded-xl border border-primary/30 bg-primary/5 p-5">
        ...
        <div className="mt-3 space-y-1.5 text-sm text-foreground/90">
          {lesson.teach.map((t, i) => <p key={i}>{t}</p>)}
        </div>
      </div>

      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="border-b border-border bg-muted/30 px-4 py-2 flex items-center justify-between">
          <div className="font-mono text-[10px] tracking-widest text-muted-foreground">{c.platform.toUpperCase()}</div>
          <div className="text-xs text-muted-foreground">{c.sender}</div>
        </div>
        <div className="p-4 space-y-1.5 text-sm">
          {c.artifact.map((line, i) => <div key={i}>{line}</div>)}
        </div>
      </div>
      ...
      {!feedback ? (
        ...
        <div className={`rounded-xl border p-5 ${feedback.correct ? "border-primary/40 bg-primary/10" : "border-caution/40 bg-caution/10"}`}>
          <div className={`font-mono text-[11px] tracking-widest ${feedback.correct ? "text-primary" : "text-caution"}`}>
