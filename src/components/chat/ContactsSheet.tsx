...
export function ContactsSheet({ open, onClose, bossId, mirrorNoHelp, onPick }: Props) {
  ...
  return (
    ...
    >
      <div className="w-full rounded-t-2xl border-t border-white/10 bg-neutral-950 max-h-[80%] overflow-hidden flex flex-col animate-in slide-in-from-bottom duration-200 shadow-2xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          <div>
            <div className="text-[10px] tracking-[0.3em] text-white/50 font-mono">
              SAVED CONTACTS
            </div>
            <div id="contacts-sheet-title" className="text-sm font-semibold text-white">
          ...
          <button
            ref={closeBtnRef}
            onClick={onClose}
            className="p-1.5 min-h-11 min-w-11 flex items-center justify-center rounded-md text-white/60 hover:text-white hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 transition-colors"
            aria-label="Close contacts sheet"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {mirrorNoHelp && (
          <div className="mx-4 mt-3 flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
            <ShieldOff className="h-4 w-4 mt-0.5 shrink-0" />
            <div>
              <div className="font-semibold text-amber-100">
                No help here — read the conversation.
              </div>
              <div className="text-amber-200/70 mt-0.5">
                Nobody in your contacts is connected to this case. Verify inside the chat.
              </div>
            </div>
          </div>
        )}
        <ul className="flex-1 overflow-y-auto p-2">
          {list.map((c) => (
            <li key={c.id}>
              <button
                onClick={() => onPick?.(c)}
                className="w-full flex items-center gap-3 rounded-lg px-3 py-3 text-left hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 transition-colors"
              >
